...
function drawCard(ctx, W, H, { kicker, title, metric }) {
  ...
  // ── kicker : point rubis + libellé en capitales tracké ──
  const kickY = y + h * 0.2
  ...
  // ── titre : 1-2 lignes, dégradé blanc → bleu froid, tracking serré ──
  const lines = Array.isArray(title) ? title : [title]
  const tSize = lines.length > 1 ? 124 : 142
  const grad = ctx.createLinearGradient(0, y + h * 0.32, 0, y + h * 0.7)
  ...
  const lh = tSize * 1.06
  const tStart = y + h * 0.49 - ((lines.length - 1) * lh) / 2
  lines.forEach((ln, i) => ctx.fillText(ln, cx, tStart + i * lh))
  ctx.letterSpacing = '0px'

  // ── proof-point + 2 filets fins ──
  const pY = y + h * 0.83
  ctx.font = '500 44px Geist, Inter, sans-serif'
  ctx.fillStyle = 'rgba(255,255,255,0.82)'
  const metricW = ctx.measureText(metric).width
  ctx.fillText(metric, cx, pY)
  ctx.strokeStyle = 'rgba(255,255,255,0.35)'
  ctx.lineWidth = 2
  const gap = metricW / 2 + 40
  ctx.beginPath()
  ctx.moveTo(cx - gap - 64, pY)
  ctx.lineTo(cx - gap, pY)
  ctx.stroke()
  ctx.beginPath()
  ctx.moveTo(cx + gap, pY)
  ctx.lineTo(cx + gap + 64, pY)
  ctx.stroke()
}
...
function useCardTexture(content) {
  const tex = useMemo(() => {
    ...
    return t
    // contenu fixe par carte → pas de deps
...
function CaveBeat({ curve, uRef, exitRef }) {
  const group = useRef()
  const mat = useRef()
...
function ExitBeat({ curve, exitRef }) {
  const group = useRef()
  const mat = useRef()
  const tex = useCardTexture({
    kicker: 'Clarity',
...
export function KeynoteCards({ curve, uRef, exitRef }) {
  return (
    <>
      <CaveBeat curve={curve} uRef={uRef} exitRef={exitRef} />
      <ExitBeat curve={curve} exitRef={exitRef} />
      <LakeBeat curve={curve} exitRef={exitRef} />
    </>
  )
}
